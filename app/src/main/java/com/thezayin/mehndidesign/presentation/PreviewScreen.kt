...
import com.thezayin.preview.presentation.components.ImagePreview
...
fun PreviewScreen(
    ...
    val likeViewModel: LikeViewModel = koinInject()
    ...
    val previewItems = viewModel.getMenuItems.collectAsState().value.list
    ...
    Scaffold(
        ...
        Column(modifier = Modifier.padding(padding)) {
            ImagePreview(url = url, items = previewItems) {
                when (it) {
                    1 -> {
                        viewModel.saveImageToStorage(url)
                    }

                    2 -> navigator.navigateUp()
                    3 -> {
                        Toast.makeText(
                            activity,
